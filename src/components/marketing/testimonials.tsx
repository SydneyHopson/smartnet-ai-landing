    ...
    <section className="relative overflow-hidden bg-[#010512] py-20 sm:py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
          <motion.article initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="group relative overflow-hidden rounded-3xl border border-sky-400/20 bg-[linear-gradient(145deg,rgba(8,17,34,.98),rgba(3,8,23,.92))] p-7 shadow-[0_28px_90px_rgba(2,6,23,.4)] sm:p-9">
          ...
          <div className="grid gap-4">
            {reviews.slice(1).map((review, index) => (
              <motion.article key={review.role} initial={{ opacity: 0, x: 18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * .06 }} whileHover={{ x: -3 }} className="group relative overflow-hidden rounded-2xl border border-sky-400/15 bg-[#07101f]/90 p-6 transition hover:border-sky-400/25">

...
export function SmartNetGeneratorSection({
  ...
  return (
    <section
      ...
      className="relative overflow-hidden border-t border-blue-500/10 bg-[#010512] py-16 sm:py-20"
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {startOnSummary && (
          <motion.div
            ...
            className="mt-6 rounded-xl border border-emerald-500/25 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-100"
          >
  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
      <div className="max-w-3xl">
        <p className="text-[0.68rem] font-semibold uppercase tracking-[0.24em] text-blue-300">
          SmartNET AI Project Builder
        </p>

        <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
          Smart solutions. Strong connections. Secure futures.
        </h2>

        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-400 sm:text-base">
          Answer a few simple questions. SmartNET turns your answers into a clear preliminary project scope and estimate.
        </p>
      </div>
...
function HeaderBadge({
  ...
  return (
    <div className="rounded-2xl border border-blue-500/15 bg-blue-500/5 px-3 py-3 text-center">
      <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-300">
        {icon}
      </div>
      <p className="mt-2 text-[0.62rem] font-semibold uppercase tracking-[0.12em] text-slate-300">
        {label}
      </p>
    </div>
  );
  ...
  return (
    <Panel accent>
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-blue-300">
        ...
        {estimator.sessionId && (
          <span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-1 text-[0.58rem] font-semibold uppercase tracking-[0.12em] text-emerald-300">
            Live
          </span>
        )}
      </div>
      ...
      <div className="mt-5">
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-500">Estimate readiness</span>
      ...
      {facts.length > 0 ? (
        <div className="mt-5 space-y-2">
          {facts.map((fact) => (
            <div
              key={fact}
              className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-950/65 px-3 py-2.5"
            >
        ...
        <p className="mt-5 text-sm leading-relaxed text-slate-500">
          Start the estimator and SmartNET will organize the project here in real time.
        </p>
      )}

      {estimator.sessionId && (
        <div className="mt-5 grid grid-cols-3 gap-2">
          <Metric label="Confirmed" value={String(completed)} />
    ...
    <Panel>
      ...
      <div className="mt-4 space-y-2">
        <SystemRow
          ...
          requested={project.cameras?.requested ?? false}
        />
        <SystemRow
          icon={<Wifi className="h-4 w-4" />}
          label="Managed Wi-Fi"
          requested={project.wifi?.requested ?? false}
        />
        <SystemRow
          icon={<Network className="h-4 w-4" />}
          label="Network"
          requested={project.network?.requested ?? false}
        />
        <SystemRow
          icon={<LockKeyhole className="h-4 w-4" />}
          label="Access Control"
          requested={project.accessControl?.requested ?? false}
        />
      </div>
    </Panel>
  );
...
function EstimatePanel() {
  ...
  const hasPricing = Boolean(
    pricing &&
      pricing.status !== "not_calculated" &&
      pricing.estimatedLow > 0 &&
      pricing.estimatedHigh > 0
  );

  return (
    <Panel accent>
      <p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-cyan-300">
      ...
      {hasPricing && pricing ? (
        <>
          <p className="mt-3 text-2xl font-semibold tracking-tight text-white">
            ${pricing.estimatedLow.toLocaleString()} – ${pricing.estimatedHigh.toLocaleString()}
          </p>

          <p className="mt-2 text-xs leading-relaxed text-slate-400">
            Final price is confirmed after walkthrough verification.
          </p>

          <div className="mt-4 grid grid-cols-2 gap-2">
            <CostCard label="Materials" value={pricing.materialCost} />
            <CostCard label="Labor" value={pricing.laborCost} />
          </div>
        </>
      ) : (
        <p className="mt-3 text-sm leading-relaxed text-slate-400">
          {readyForPricing
            ? "SmartNET has enough information to calculate the preliminary estimate."
            : "Pricing will appear when discovery is complete."}
        </p>
      )}

      {readyForPricing && !estimator.proposalId && (
        <button
          type="button"
          disabled={estimator.isFinalizing}
          onClick={() => void finalizeEstimator()}
          className="mt-5 w-full rounded-xl bg-gradient-to-r from-blue-600 to-cyan-400 px-4 py-3 text-sm font-semibold text-white shadow-[0_0_24px_rgba(37,99,235,0.3)] transition hover:brightness-110 disabled:opacity-50"
        >
          {estimator.isFinalizing
      ...
      {estimator.proposalId && (
        <div className="mt-4 rounded-xl border border-emerald-500/25 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-100">
          Proposal ready.
        </div>
      )}
  ...
  return (
    <Panel>
      <div className="flex items-start gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-300">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-slate-500">
            Measurements, pathways, mounting conditions, power, and hardware are verified during the walkthrough.
          </p>
        </div>
      </div>
    </Panel>
  ...
  return (
    <motion.section
      ...
      className={`rounded-2xl border p-4 sm:p-5 ${
        accent
          ? "border-blue-500/25 bg-[radial-gradient(circle_at_top,rgba(0,102,255,0.12),rgba(3,8,23,0.98))] shadow-[0_0_28px_rgba(0,102,255,0.1)]"
          : "border-slate-800 bg-[#030817]/90"
      }`}
    >
      {children}
    </motion.section>
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/70 px-2 py-2.5 text-center">
      <p className="text-[0.55rem] uppercase tracking-[0.12em] text-slate-600">
        {label}
      </p>
...
function SystemRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-950/65 px-3 py-3">
      <span
      ...
      <span
        className={`rounded-full border px-2 py-1 text-[0.55rem] font-semibold uppercase tracking-[0.1em] ${
          requested
            ? "border-emerald-500/25 bg-emerald-500/10 text-emerald-300"
            : "border-slate-700 bg-slate-900 text-slate-600"
        }`}
      >
        {requested ? "Included" : "Not selected"}
      </span>
    </div>
  );
}
...
function CostCard({
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
      <p className="text-[0.56rem] uppercase tracking-[0.12em] text-slate-600">
        {label}
      </p>
...
function calculateProgress(
  ...
  return Math.max(
    ...
    Math.min(
      100,
      Math.round(questionProgress * 0.7 + confidence * 0.3)
    )
  );
}

...
function wifi(p: ProjectEstimate, users: number, outdoor = false) { p.network.requested = true; p.wifi.requested = true; p.wifi.estimatedConcurrentUsers = q(users); p.wifi.indoorCoverage = true; p.wifi.outdoorCoverage = outdoor; p.network.existingRouter = false; p.network.existingSwitches = false; p.network.existingRack = false; p.network.rackRequired = true; }

...
export function ServiceAreaSection() {
  return (
    <section className="relative overflow-hidden bg-[#010512] py-20 sm:py-24">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_80%_50%,rgba(37,99,235,.15),transparent_30rem)]" />
      <div className="relative mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {cities.map((city) => <span key={city} className="rounded-full border border-sky-400/15 bg-sky-400/[.06] px-3 py-1.5 text-xs font-medium text-slate-300">{city}</span>)}
          </div>

          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-sky-400/15 bg-[#07101f]/80 p-4"><Navigation className="h-5 w-5 text-sky-300" /><p className="mt-3 text-sm font-semibold text-white">Residential & commercial</p><p className="mt-1 text-xs leading-5 text-slate-500">Homes, offices, retail and light industrial projects.</p></div>
            <div className="rounded-2xl border border-sky-400/15 bg-[#07101f]/80 p-4"><Building2 className="h-5 w-5 text-sky-300" /><p className="mt-3 text-sm font-semibold text-white">Multi-site review</p><p className="mt-1 text-xs leading-5 text-slate-500">Larger and out-of-area opportunities can be scoped separately.</p></div>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, scale: .97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative min-h-[420px] overflow-hidden rounded-3xl border border-sky-400/20 bg-[linear-gradient(145deg,rgba(8,17,34,.98),rgba(2,6,23,.96))] p-6 shadow-[0_28px_90px_rgba(2,6,23,.45)]">
          ...
          <div className="relative mt-8 flex min-h-[280px] items-center justify-center">
            <div className="absolute h-64 w-64 rounded-full border border-sky-400/15" />
            <div className="absolute h-44 w-44 rounded-full border border-sky-400/25" />
            <div className="absolute h-24 w-24 rounded-full border border-sky-400/40 bg-sky-400/[.04]" />
            <div className="absolute h-px w-[85%] bg-gradient-to-r from-transparent via-sky-400/30 to-transparent" />
            <div className="absolute h-[85%] w-px bg-gradient-to-b from-transparent via-sky-400/30 to-transparent" />

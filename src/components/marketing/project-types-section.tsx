...
const projects = [
  { icon: Home, eyebrow: "Residential", title: "Homes & estates", body: "Driveways, porches, garages, backyards and interior coverage designed around how the property is actually used.", tag: "PERIMETER / WIFI", position: "object-[64%_52%]" },
  { icon: Store, eyebrow: "Retail", title: "Shops, salons & studios", body: "Customer entrances, registers, service areas and back rooms covered without turning the space into a camera showroom.", tag: "POS / ENTRY", position: "object-[52%_44%]" },
  { icon: Warehouse, eyebrow: "Industrial", title: "Warehouses & yards", body: "Dock doors, aisles, loading lanes and perimeter coverage with cable paths planned for serviceability and growth.", tag: "DOCK / AISLE", position: "object-[72%_58%]" },
  { icon: Building2, eyebrow: "Commercial", title: "Offices & suites", body: "Lobbies, corridors, shared spaces, Wi-Fi and IT closets tied together as one clean system.", tag: "LAN / ACCESS", position: "object-[48%_55%]" },
  { icon: Church, eyebrow: "Community", title: "Churches & gathering spaces", body: "Sanctuaries, classrooms, parking lots and common areas planned with coverage, comfort and privacy in mind.", tag: "COVERAGE / AV", position: "object-[38%_50%]" },
  { icon: KeyRound, eyebrow: "Hospitality", title: "Rentals & small portfolios", body: "Entry, parking and common-area visibility for short-term rentals and multi-property owners.", tag: "ENTRY / REMOTE", position: "object-[58%_48%]" },
];
          ...
          {projects.map((item, index) => {
            ...
            return (
              <motion.article key={item.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: .35, delay: index * .04 }} whileHover={{ y: -6 }} className="group relative min-h-[330px] overflow-hidden rounded-2xl border border-sky-400/15 bg-[#030817] shadow-[0_20px_70px_rgba(2,6,23,.42)]">
                <Image src="/hero/images/smartnet-ai-building-v3.png" alt="" fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" className={`object-cover ${item.position} opacity-35 saturate-75 transition duration-700 group-hover:scale-105 group-hover:opacity-50 group-hover:saturate-100`} />
                <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(2,6,23,.12)_0%,rgba(2,6,23,.55)_38%,rgba(2,6,23,.98)_78%)]" />
                ...
                <div className="relative flex items-start justify-between gap-4 p-5">
                  ...
                  <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-3 py-1.5 backdrop-blur"><span className="text-[.55rem] font-bold tracking-[.16em] text-slate-300">{item.tag}</span><ArrowUpRight className="h-3.5 w-3.5 text-sky-300" /></div>
                </div>

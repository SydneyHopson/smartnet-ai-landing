...
const tiers = [
  { icon: Home, label: "Residential", title: "Smart home coverage", range: "Preliminary range after AI discovery", body: "Cameras, Wi-Fi improvements, clean routing and setup sized to the home and actual coverage goals.", image: "/trustbar/smartnet-badge.png" },
  { icon: Building2, label: "Business", title: "Commercial systems", range: "Scoped around site complexity", body: "Interior/exterior coverage, network improvements and access-control options for customer-facing and staff spaces.", featured: true, image: "/hero/images/smartnet-ai-building-v3.png" },
  { icon: Warehouse, label: "Larger sites", title: "Warehouse & perimeter", range: "Custom project range", body: "Longer runs, high mounting, larger camera counts, racks and network infrastructure planned around the site.", image: "/trustbar/smartnet-badge21.png" },
];
          ...
          {tiers.map((tier, index) => {
            ...
            return (
              <motion.article key={tier.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .05 }} whileHover={{ y: -7 }} className={`group relative min-h-[390px] overflow-hidden rounded-2xl border ${tier.featured ? "border-sky-300/40 shadow-[0_0_70px_rgba(37,99,235,.18)]" : "border-sky-400/15"}`}>
                <Image src={tier.image} alt="" fill sizes="(max-width:1024px) 100vw, 33vw" className={`object-cover opacity-35 transition duration-700 group-hover:scale-105 group-hover:opacity-50 ${index === 1 ? "object-[70%_45%]" : "object-center"}`} />
                <div className="absolute inset-0 bg-gradient-to-t from-[#020617] via-[#041024]/85 to-[#041024]/20" />

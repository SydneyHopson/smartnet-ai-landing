...
export function HeroSection() {
  const handleStartEstimateClick = () => {
    ...
    if (estimator) estimator.scrollIntoView({ behavior: "smooth", block: "start" });
  };

  const handleScheduleWalkthroughClick = () => {
    const bookingCalendar = document.getElementById("booking-calendar");
    if (bookingCalendar) bookingCalendar.scrollIntoView({ behavior: "smooth", block: "start" });
  };
      ...
      <div className="relative z-10 mx-auto max-w-[1680px] px-5 sm:px-8 lg:px-10">
        <header className="mb-12 flex flex-col gap-8 border-b border-sky-500/15 pb-8 xl:flex-row xl:items-center xl:justify-between">
          <div className="relative h-24 w-[320px] max-w-full sm:h-28 sm:w-[380px]">
            ...
            <Image src="/logos/images/smartnet-installation-logo-2026.png" alt="SmartNET Installation LLC" fill priority sizes="(max-width: 640px) 320px, 380px" className="relative object-contain object-left drop-shadow-[0_0_24px_rgba(56,189,248,.45)]" />
          </div>

  ...
  try {
    ...
    if (!contact?.fullName?.trim() || !contact?.email?.trim() || !contact?.phone?.trim() || !body.dateISO || !body.timeSlot) {
      return NextResponse.json(
        { error: "Missing required fields (name, email, phone, date, timeSlot)" },
        { status: 400 }
      );
    }
    ...
    const roughRangeLabel = low !== null || high !== null
      ? `$${low?.toLocaleString() ?? "?"} – $${high?.toLocaleString() ?? "?"}`
      : "No AI estimate attached";

    const locationLabels: Record<BookingPayload["jobLocation"]["type"], string> = {
      home: "Home / residence",
      office: "Office / suite",
      retail: "Retail / restaurant / storefront",
    ...
    const bookingDoc = await sanityWriteClient.createIfNotExists({
      ...
      // Lossless project snapshot. This carries cameras, APs, access control,
      // cabling, installation, BOM, pricing and assessment into operations.
      rawEstimateJson: estimate ? JSON.stringify(estimate) : null,
      createdAt: createdAt.toISOString(),
    });

    if (alreadyExists) {
      return NextResponse.json({ ok: true, id: bookingDoc._id, deduped: true }, { status: 200 });
    }
    ...
    const datePretty = new Date(body.dateISO).toLocaleDateString("en-US", {
      dateStyle: "full",
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#0f172a;line-height:1.5;">
            <h2 style="margin-bottom:4px;">New SmartNET Booking</h2>
            <p><strong>${escapeHtml(appointmentType)}</strong> · ${escapeHtml(datePretty)} · ${escapeHtml(body.timeSlot)}</p>
            <p><strong>Client:</strong> ${escapeHtml(contact.fullName)}<br/>
            <strong>Email:</strong> ${escapeHtml(contact.email)}<br/>
            <strong>Phone:</strong> ${escapeHtml(contact.phone)}<br/>
            <strong>Location:</strong> ${escapeHtml(locationTypeLabel)}</p>
            ${body.jobLocation?.note ? `<p><strong>Site notes:</strong> ${escapeHtml(body.jobLocation.note)}</p>` : ""}
            <hr style="border:none;border-top:1px solid #e2e8f0;margin:18px 0;"/>
            <p><strong>AI estimate:</strong> ${estimate ? "Attached" : "Not completed — direct booking"}<br/>
            <strong>Range:</strong> ${escapeHtml(roughRangeLabel)}<br/>
            <strong>Scope:</strong> ${escapeHtml(estimateSummary)}</p>
            <p style="background:#f1f5f9;padding:12px;border-radius:8px;">${escapeHtml(appointmentGuidance)}</p>
            <p><a href="${ownerBookingUrl}" style="font-weight:700;color:#0369a1;">Open booking record</a></p>
          </div>
        `,
      });
    } catch (emailErr) {
    ...
    return NextResponse.json({ ok: true, id: bookingDoc._id }, { status: 200 });
  } catch (err) {

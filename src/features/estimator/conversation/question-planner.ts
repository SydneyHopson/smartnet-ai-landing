...
export type EstimatorQuestion = {
  ...
  category: "project" | "property" | "cameras" | "network" | "wifi" | "access_control" | "cabling" | "installation" | "commercial";
};
...
function addQuestion(questions: EstimatorQuestion[], question: EstimatorQuestion): void {
  if (!questions.some((existing) => existing.key === question.key)) questions.push(question);
}
...
export function getEstimatorQuestions(project: ProjectEstimate): EstimatorQuestion[] {
  ...
  if (!hasKnownQuantity(project.property.squareFootage)) addQuestion(questions, { key: "property.squareFootage", question: "About how many square feet is the project area?", reason: "Square footage drives labor and equipment estimates.", priority: "critical", category: "property" });
  ...
  if (project.property.ceilingType === "unknown") addQuestion(questions, { key: "property.ceilingType", question: `What kind of ceilings are in the ${language.area}?`, reason: "Ceiling type affects cable installation.", priority: "high", category: "property" });
  if (!hasKnownQuantity(project.property.ceilingHeightFeet)) addQuestion(questions, { key: "property.ceilingHeightFeet", question: "Approximately how tall are the ceilings where equipment will be installed?", reason: "Ceiling height determines access equipment and labor.", priority: "high", category: "property" });
  ...
  if (networkingIsRequired) {
    ...
    if (project.network.existingRack === null) addQuestion(questions, { key: "network.existingRack", question: project.property.projectType === "residential" ? "Do you already have a network cabinet, structured wiring panel, or small rack?" : "Do you already have a network rack or cabinet?", reason: "Determines whether rack hardware needs to be included.", priority: "normal", category: "network" });
    if (project.network.existingRack === false && project.network.rackRequired === null) addQuestion(questions, { key: "network.rackRequired", question: project.property.projectType === "residential" ? "Would you like SmartNET to provide a small cabinet or rack to organize the new equipment?" : "Would you like SmartNET to provide a network rack?", reason: "Rack hardware changes equipment pricing.", priority: "normal", category: "network" });
  }

  if (project.wifi.requested) {
    if (project.wifi.coverageGoals.length === 0) addQuestion(questions, { key: "wifi.coverageGoals", question: project.property.projectType === "residential" ? "Where around the home do you need reliable Wi-Fi coverage?" : `Where in or around the ${language.area} do you need reliable Wi-Fi coverage?`, reason: "Coverage area determines access-point quantity and placement.", priority: "critical", category: "wifi" });
    ...
    if (project.wifi.guestNetworkRequired === null) addQuestion(questions, { key: "wifi.guestNetworkRequired", question: project.property.projectType === "residential" ? "Would you like a separate guest Wi-Fi network for visitors?" : "Do you want a separate guest Wi-Fi network?", reason: "Guest Wi-Fi may require additional configuration.", priority: "normal", category: "wifi" });
  }
  ...
  if (cablingIsRequired) {
    if (project.cabling.existingCablingAvailable === null) addQuestion(questions, { key: "cabling.existingCablingAvailable", question: "Can we reuse any of the existing network cabling?", reason: "Reusing cabling can significantly reduce labor and material costs.", priority: "high", category: "cabling" });
    if (project.cabling.pathwayType.length === 0) addQuestion(questions, { key: "cabling.pathwayType", question: project.property.projectType === "residential" ? "What cable pathways are available, such as attic, basement, crawlspace, or accessible utility areas?" : "How will new cables most likely be run?", reason: "Cable pathways have the biggest impact on labor.", priority: "critical", category: "cabling" });
  }

  if (cablingIsRequired && project.installation.liftRequired === null) addQuestion(questions, { key: "installation.liftRequired", question: "Will any installation area be higher than a normal ladder can safely reach?", reason: "Lift equipment greatly affects labor and equipment pricing.", priority: "high", category: "installation" });
  if (cablingIsRequired && project.installation.afterHoursRequired === null) addQuestion(questions, { key: "installation.afterHoursRequired", question: `Does the work need to happen outside ${language.hours}?`, reason: "Special scheduling or after-hours work can increase labor cost.", priority: "normal", category: "installation" });

  const isCommercial = project.property.projectType !== null && project.property.projectType !== "residential";
  ...
  return questions.sort((first, second) => {
    ...
    return priorityDifference !== 0 ? priorityDifference : first.key.localeCompare(second.key);
  });
}
